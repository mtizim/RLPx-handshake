...
pub async fn initiate(address: impl ToSocketAddrs, recipient_pubkey: PublicKey) -> Result<()> {
    ...
    let (secrets, recipient_nonce, ack_bytes) = receive_auth_ack(
        &mut stream,
        &identity,
        &ephemeral_privkey,
        &recipient_pubkey,
    ...
    let mut state = P2PState::new(
        ...
        secrets,
        initiator_nonce,
        H256::from_slice(&recipient_nonce),
        &auth_bytes,
        &ack_bytes,
    );
...
async fn receive_auth_ack(
...
) -> Result<(AuthSecrets, [u8; 32], Vec<u8>)> {
    let buf = &mut [0u8; 512];
    ...
    Ok((secrets, body.recipient_nonce, ack_bytes))
}
...
async fn send_auth_message(
    ...
    let ecdh = ecdh(&identity.secret, recipient_pubkey);
    ...
    AuthBody {
        sig: sig_bytes,
        initiator_pubk: identity.public.serialize_uncompressed()[1..]
            .try_into()
            .expect("65 - 1 == 64"),
